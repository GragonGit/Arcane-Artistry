...
/**
 * Params: {@code effect} (a status effect id, e.g. "minecraft:speed"),
 * {@code duration} (ticks), {@code amplifier}.
 */
public final class SelfBuffSpellEffect implements SpellEffect {
  ...
  public void execute(SpellEffectContext context, JsonObject params) {
    Identifier effectId = Identifier.parse(params.get("effect").getAsString());
    ...
    var entry = BuiltInRegistries.MOB_EFFECT.get(effectId).orElse(null);
    if (entry == null) {
      ArcaneArtistryBase.LOGGER.warn("Spell {} references unknown status effect {}", context.spell().id(), effectId);
      return;
    }

    context.caster().addEffect(new MobEffectInstance(entry, duration, amplifier));
  }
}
