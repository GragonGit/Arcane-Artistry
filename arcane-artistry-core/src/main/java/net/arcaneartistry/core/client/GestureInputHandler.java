 ...
 *
 * <p><b>Deliberately implemented without a mixin into raw mouse input.</b>
 ...
 * start of the tick.
 *
 * <p>That reset both "locks the camera" (no net rotation reaches the
 ...
 *
 * <p>This trades a small amount of directness for relying only on APIs
 * ({@code Entity#getYaw/setYaw/getPitch/setPitch}, Fabric's
...
public final class GestureInputHandler {
    ...
    public void startCast(ClientPlayerEntity player, ItemStack stack) {
        casting = true;
    ...
    public void endCast(ClientPlayerEntity player, ItemStack stack, Hand hand) {
        if (!casting) {
            return;
        }
        casting = false;
    ...
    void onEndClientTick(MinecraftClient client) {
        ...
        double deltaYaw = MathHelper.wrapDegrees(player.getYaw() - lockedYaw);
        double deltaPitch = player.getPitch() - lockedPitch;
    ...
    private void appendStroke(ClientPlayerEntity player, GestureDirection direction) {
